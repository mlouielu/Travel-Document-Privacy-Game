...
export const AppScreenshotCard = ({ details, showLeak, leakTarget, onInteract, isSafe }) => {
  ...
  return (
    <div className="relative w-[280px] h-[500px] bg-white rounded-[30px] border-[8px] border-gray-800 overflow-hidden shadow-xl transform scale-90 sm:scale-100 origin-center">
      ...
      <div className="px-4 space-y-4">
        {/* Flight Card */}
        <div 
            onClick={() => onInteract && onInteract('flight_card')}
            className={`bg-gray-50 p-3 rounded-lg border shadow-sm transition-all duration-300 
                ${onInteract ? 'cursor-pointer hover:bg-blue-50/50' : ''}
                ${isLeakVisible('flight_card') || isLeakVisible('flight') || isLeakVisible('route') || isLeakVisible('date')
                    ? 'bg-red-500/10 border-red-500 ring-2 ring-red-500 animate-pulse' 
                    : 'border-gray-200'}
            `}
        >
             ...
             {(isLeakVisible('flight') || isLeakVisible('route') || isLeakVisible('date')) && (
                <div className="mt-1 flex justify-center">
                    <span className="bg-red-600 text-white text-[9px] px-1.5 py-0.5 rounded font-bold shadow">
                        {t('card.secret_alert')}
                    </span>
                </div>
             )}
        ...
        <div className="space-y-3">
             <div 
                ...
                className={`border-b transition-all duration-300 rounded px-1 -mx-1
                    ${onInteract ? 'cursor-pointer hover:bg-blue-50/50' : ''}
                    ${isLeakVisible('passenger') ? 'bg-red-500/10 border-red-500 ring-1 ring-red-500' : 'border-gray-100'}
                `}
             >
             ...
             <div 
                ...
                className={`relative border-b transition-all duration-300 rounded px-1 -mx-1
                    ${onInteract ? 'cursor-pointer hover:bg-blue-50/50' : ''}
                    ${isLeakVisible('pnr') ? 'bg-red-500/10 border-red-500 ring-2 ring-red-500' : 'border-gray-100'}
                `}
             >
                <div className="text-xs text-gray-400 uppercase">{t('card.pnr_full_label')}</div>
                <div className={`font-mono font-bold text-sm tracking-widest ${isSafe && !isLeakVisible('pnr') ? 'blur-sm' : ''}`}>{pnr}</div>
